...
public class PolyphenyConnection {
    ...
    public PolyphenyConnection( String url, String username, String password ) throws Exception {
        ...
        this.connection = null; // The connection is established later on when needed. Lazy open
                               // prevents
                               // accidental resource leaks induced by user
        StartLocalPolypheny(); // Starts Polypheny locally on the machine if it's not already
                              // running
    ...
    public void StartLocalPolypheny() {
        try {
            if ( !isPolyphenyRunning() ) {
                System.out.println(
                        "Polypheny not running. Attempting to start Polypheny application" );
