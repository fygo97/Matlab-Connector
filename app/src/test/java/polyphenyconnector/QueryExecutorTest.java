...
public class QueryExecutorTest {
    ...
    @Test
    void testQueryWithSpacesPostgreSQL() {
        myexecutor.execute( "sql", "CREATE TABLE unittest_namespace.pg_table (id INT NOT NULL, name VARCHAR(100), PRIMARY KEY (id)) ON STORE postgres" );
        myexecutor.execute( "sql", "ALTER TABLE unittest_namespace.unittest_table ADD PLACEMENT ON postgres_adapter_name;" );

        // Insert id = 1,2 and name = Alice, Bob into the table.

        myexecutor.execute( "sql", "  INSERT INTO unittest_namespace.unittest_PostgresTable VALUES (1, 'Alice') USING PostgreSQL" );
        myexecutor.execute( "sql", " INSERT INTO unittest_namespace.unittest_PostgresTable VALUES (2, 'Bob')" );

        // Query the result from the table.
        Object result = myexecutor.execute( "sql", "SELECT id, name FROM unittest_namespace.unittest_PostgresTable ORDER BY id" );
        ...
        myexecutor.execute( "sql", "DROP TABLE IF EXISTS unittest_namespace.unittest_PostgresTable" );
