...
public class QueryExecutorTestMQL {
    ...
    static void setUpNamespaceAndCollection() throws Exception {
        ...
        try {
            myexecutor.executeSql( "CREATE DOCUMENT NAMESPACE IF NOT EXISTS mongotest" );
            myexecutor.executeMongo( "mongo", "mongotest", "db.createCollection(\"unittest_collection\")" );
    ...
    static void tearDownNamespaceAndCollection() {
        try {
            myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.drop()" );
            myexecutor.executeSql( "DROP NAMESPACE IF EXISTS mongotest" );
    ...
    void clearCollection() {
        try {
            myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.deleteMany({})" );
        } catch ( Exception ignored ) {
        }
    }
    ...
    void testDeleteManyRemovesAllDocs() {
        ...
        // Insert 3 docs
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"id\":1,\"name\":\"Alice\"})" );
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"id\":2,\"name\":\"Bob\"})" );
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"id\":3,\"name\":\"Ciri\"})" );

        // Act: delete all
        Object ack = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.deleteMany({})" );

        // Assert: ack JSON contains deletedCount:3
        assertTrue( ack.toString().contains( "\"updateCount\":3" ), "Expected 3 deletions, got: " + ack );
        // Verify collection is empty
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
    ...
    void testInsertandDrop() {
        Object result = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
        assertTrue( result instanceof List, "Expected a List<String>" );
        List<String> docs = (List<String>) result;
        assertEquals( 0, docs.size() );

        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"age\":14})" );
        result = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({\"age\":14})" );
        ...
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.deleteMany({})" );
        result = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({\"age\": {\"$eq\": 14}})" );
        docs = (List<String>) result;
        assertEquals( 0, docs.size() );
    }
    ...
    void testInsertManyAndNestedDocument() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"age\":14})" );
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"age\":20})" );
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"age\":24})" );
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"age\":30, \"adress\": {\"Country\": \"Switzerland\", \"Code\": 4051}})" );
        Object result = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({\"age\": {$gt:29}})" );
    ...
    void testBooleanField() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"flag\":true})" );
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
    ...
    void testIntegerAgeField() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"age\":42})" );
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
        assertEquals( 1, docs.size() );
    ...
    void testStringField() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"name\":\"Alice\"})" );
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
        assertEquals( 1, docs.size() );
    ...
    @Test
    void testLongField() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"big\":1111111111111111111})" );
        Object r = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
    ...
    void testDoubleField() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"pi\":3.14159})" );
        Object r = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) r;
        assertEquals( 1, docs.size() );
    ...
    void testCountDocumentsReturnsStringArray() {
        myexecutor.executeMongo( "mongo", "mongotest",
                "db.unittest_collection.insertOne({\"name\":\"Bob\"})" );

        Object result = myexecutor.executeMongo( "mongo", "mongotest",
        ...
        assertEquals( 1, docs.size() );
        System.out.println( docs.get( 0 ) );
        assertEquals( "{\"count\":1.0}", docs.get( 0 ) );
    }


    @Test
    void testListElementClasses() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"mixed\":[{\"bar\":2},1,\"foo\"]})" );
        Object result = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) result;
        System.out.println( docs.get( 0 ) );
    }
    ...
    @Test
    void testArrayField() {
        myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.insertOne({\"scores\":[1,2,3]})" );
        Object result = myexecutor.executeMongo( "mongo", "mongotest", "db.unittest_collection.find({})" );
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) result;
        assertEquals( 1, docs.size() );
        assertTrue( docs.get( 0 ).contains( "\"scores\":[1.0,2.0,3.0]" ) );
    }
    ...
    void testSyntaxErrorThrows() {
        ...
        RuntimeException runtimeException = assertThrows( RuntimeException.class, () -> {
            myexecutor.executeMongo( "mongo", "mongotest", badQuery );
        } );
    ...
    void testMultiStatementMongoFails() {
        ...
        assertThrows( Exception.class, () -> {
            myexecutor.executeMongo( "mongo", "mongotest", illegal_multiquery );
        }, "Polypheny should not support multi-statement MongoQL with ';'" );
    }

    /* CAVE: this test fails since I added myexecutor.executeSql( "DROP NAMESPACE IF EXISTS mongotest" ); in "@AfterAll"
    @Test
    ...
    */
    ...
    void testExecuteBatchMongoRollback() {
        ...
        @SuppressWarnings("unchecked") List<String> docs = (List<String>) myexecutor.executeMongo(
